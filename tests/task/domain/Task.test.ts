...
test("a Task is created with all the required parameters", () => {
	...
	const task = new Task(id, taskName, taskDescription, status, userTaskCreator, startDate, null);
...
test("if not all the required properties are introduced, an error is thrown", () => {
	...
	expect(() => {
		new Task(id, taskName, taskDescription, status, userTaskCreator, startDate, null);
